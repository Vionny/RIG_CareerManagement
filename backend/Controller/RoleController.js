...
const getAstRegisteredRole = (req, res, next) =>{
    ...
    const query = "SELECT * FROM role WHERE roleid NOT IN ( SELECT roleid FROM promotionregistrationdetail prd JOIN promotionregistration pr ON pr.promotionregistrationid = prd.promotionregistrationid  WHERE initial = $1' AND semesterid <> $2 ) ORDER BY roleid"
